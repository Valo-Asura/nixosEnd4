...
Rectangle {
    ...
    Popup {
        ...
        property point originPoint: {
            if (!root.window || !categoryOpenButtonLoader.window)
                return Qt.point(0, 0);

            return categoryOpenButtonLoader.mapToItem(root, categoryOpenButtonLoader.width / 2, categoryOpenButtonLoader.height / 2);
        }
        property point windowCenterPoint: {
            const rootContentItem = root.windowRootItem;
            const sectionItem = root.parent?.parent?.parent;
            if (!root.window || !rootContentItem || !sectionItem)
                return Qt.point(root.width / 2, root.height / 2);
    ...
    component SmallGridAppButton: SmallGridButton {
        ...
        onClicked: {
            ...
            LauncherApps.launchDesktopEntry(desktopEntry);
        }
